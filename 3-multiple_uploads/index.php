...
<body>
    ...
    if (isset($_POST['enviar-form'])) {
        ...
        while ($contador < $quantidadeArquivos) {
            $extensao = pathinfo($_FILES["arquivo"]["name"][$contador], PATHINFO_EXTENSION); // O terceiro parâmetro é o indice do arquivo
            if (in_array($extensao, $formatosPermitidos)) { // Verifica se o parâmetro 1 está no parâmetro 2 (array)
                $pasta = "files/";
                $temporario = $_FILES['arquivo']['tmp_name'][$contador]; //Pega o caminho temporário do arquivo enviado (primeiro parametro)
                ...
                if (move_uploaded_file($temporario, $pasta . $newName)) //Move o arquivo enviado para o segundo parâmetro, nesse caso a pasta files, junto com o id e sua extensão
                    echo "Upload feito com sucesso para $pasta.$newName<br>";
                else
                    echo "Erro ao enviar o arquivo $temporario<br>";
            } else
                echo "Arquivo com extensão .$extensao não são permitidos!<br>";
            $contador++;
        }
    }
    ?>
